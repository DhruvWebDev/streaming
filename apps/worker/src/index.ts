...
function getCommandToSpinEcsTask(config: ECSConfig, inputVideo: string, bucketName: string, output: string): RunTaskCommand {
    ...
    try {
        ...
        if (response.Messages && response.Messages.length > 0) {
            for (const message of response.Messages) {
                try {
                    const body: SQSMessageBody | undefined = message?.Body ? JSON.parse(message.Body) : undefined;
                    if (body) {
                            ...
                            for (const record of body.Records) {
                                ...
                                if (record.eventName === "ObjectCreated:Put") {
                                    ...
                                    const command = getCommandToSpinEcsTask(
                                        { CLUSTER: 'arn:aws:ecs:eu-north-1:982081060540:cluster/build-video', TASK: 'arn:aws:ecs:eu-north-1:982081060540:task-definition/transcode-video:1' },
                                        inputVideo, 
                                        s3Bucket, 
                                        'output'
                                    );
